import { ColumnDef } from "@tanstack/react-table";
import Image from "next/image";
import { formatDate } from "@/lib/utils";
import { TABLE_COLUMN_SIZES } from "@/constants";
import { getBgColor, getTextColor } from "@/lib/tableUtils";
import { PillButton } from "@/components/ui/PillButton";
import { TableHeader } from "./utils";
import { ActiveVideoTableTypes } from "@/types/restaurant";
import { ApprovalStatusEnum } from "@/types";

/**
 * Creates column definitions for pending videos table
 */
export const createActiveVideosColumns = (
  handleOpenVideoPreview: (locationId: string, submissionId: string) => void
): ColumnDef<ActiveVideoTableTypes>[] => [
  {
    accessorKey: "id",
    header: () => <TableHeader title="#" />,
    cell: ({ row, table }) => {
      const { pageSize, pageIndex } = table.getState().pagination;
      const rowIndex = row.index + 1;
      const calculatedId = pageIndex * pageSize + rowIndex;

      return (
        <div className="font-medium text-[#181D1F] w-fit truncate">
          {calculatedId}
        </div>
      );
    },
    minSize: TABLE_COLUMN_SIZES.ID,
    meta: { sticky: true },
  },
  {
    accessorKey: "name",
    header: () => <TableHeader title="Restaurant name" />,
    cell: ({ row }) => {
      const restaurantName = row.original.location?.name ?? "";

      return (
        <div
          role="button"
          tabIndex={0}
          aria-label="View restaurant details"
          className="absolute top-0 bottom-0 left-0 right-0 flex items-center py-5 px-2.5 cursor-pointer font-medium text-[#181D1F] max-w-60 w-full h-full overflow-hidden"
        >
          <span className="truncate w-full">{restaurantName}</span>
        </div>
      );
    },
    minSize: TABLE_COLUMN_SIZES.NAME,
    meta: { sticky: true },
  },
  {
    accessorKey: "posts",
    header: () => <TableHeader iconName="videoIcon" title="Video" />,
    cell: ({ row }) => {
      const postCount = 1;

      return (
        <div className="flex flex-col gap-y-2 w-fit">
          <PillButton
            text={`${postCount} video${postCount > 1 ? "s" : ""}`}
            textColor={getTextColor([{ status: ApprovalStatusEnum.APPROVED }])}
            bgColor={getBgColor([{ status: ApprovalStatusEnum.APPROVED }])}
          />
          <span
            className="text-left text-blue-500 hover:underline hover:cursor-pointer"
            onClick={() =>
              handleOpenVideoPreview(
                row.original?.location?.id ?? "",
                row.original?.submissionId ?? ""
              )
            }
          >
            View
          </span>
        </div>
      );
    },
    minSize: TABLE_COLUMN_SIZES.VIDEO,
  },
  {
    accessorKey: "foodVisible",
    header: () => <TableHeader iconName="statusIcon" title="Food visible" />,
    cell: ({ row }) => {
      const isFoodVisible = row.original.isFoodVisible;
      return (
        <div className="relative">
          <div className="flex items-end w-full gap-2 px-2 py-1 text-left">
            <span className="text-[#181D1F] font-medium">
              {isFoodVisible ? "Yes" : "No"}
            </span>
          </div>
        </div>
      );
    },
    minSize: TABLE_COLUMN_SIZES.VIDEO,
  },
  {
    accessorKey: "addedBy",
    header: () => <TableHeader iconName="calendarIcon" title="Added by" />,
    cell: ({ row }) => {
      const addedBy =
        row.original.admin?.name ?? row.original.user?.displayName;
      return (
        <div className="flex items-center w-full gap-2 px-2 py-1 text-left">
          <Image
            src={"/images/user.png"}
            alt={addedBy ?? "Gontrel"}
            width={40}
            height={40}
            className="object-cover rounded-full"
          />
          <span className="font-medium text-black">{addedBy ?? "James"}</span>
        </div>
      );
    },
    minSize: TABLE_COLUMN_SIZES.ADDED_BY,
  },
  {
    accessorKey: "dateAdded",
    header: () => <TableHeader iconName="calendarIcon" title="Date added" />,
    cell: ({ row }) => {
      const dateAdded = new Date(
        row.original.submissionDate ?? row.original?.createdAt ?? ""
      );

      return (
        <div className="relative">
          <div className="flex items-center w-full gap-2 px-2 py-1 text-left">
            <span className="text-[#181D1F] font-medium">
              {formatDate(dateAdded)}
            </span>
          </div>
        </div>
      );
    },
    minSize: TABLE_COLUMN_SIZES.DATE_ADDED,
  },
  {
    accessorKey: "dateModified",
    header: () => <TableHeader iconName="calendarIcon" title="Date modified" />,
    cell: ({ row }) => {
      const dateModified = new Date(row.original.modifiedAt ?? "");

      return (
        <div className="relative">
          <div className="flex items-center w-full gap-2 px-2 py-1 text-left">
            <span className="text-[#181D1F] font-medium">
              {formatDate(dateModified)}
            </span>
          </div>
        </div>
      );
    },
    minSize: TABLE_COLUMN_SIZES.DATE_ADDED,
  },
];
